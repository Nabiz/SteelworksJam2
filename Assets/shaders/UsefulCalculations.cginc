...
float3 PerlinNoise(float3 p, float4 s)
{
	...
	//return float3(noise(p.x), 0, 0);
	return float3(noisee(p.x), noisee(p.y), noisee(p.z));
	return float3(noisee(round(p.x / s.x) * s.x), noisee(round(p.y / s.y) * s.y), noisee(round(p.z / s.z) * s.z));
}
...
//Work inprogress
float MetalText(float3 p, float4 s)
{

	//p = Rotate3D(p, GradientNoise(p) * 360);
	//p = GradientNoise(p, s * 3);
	//p = Rotate3D(p, noise(round(p / s) * s) * 360);

	p.x += 10;
	p.z += 10;
	float result = p.y / 3 + GradientNoise(p.z * sin(GradientNoise(p.x)) + GradientNoise(p.x)) ;
	//result = ;
	return result;
}